...
console.log("VALOR MAS ALTO DE VENTA:", historyGetHighestSellPrice());
console.log("VALOR MAS BAJO DE COMPRA:", historyGetLowestBuyPrice());

function getEUR(callback){
    return authedClient.getAccounts((error, response, data) => {
        if (error) {
            console.log(error);
        } else {
            callback(data.filter((el) => {
                return el.currency == 'EUR'
            }));
        }
    });
}
...
async function buy(data, market, trans) {
    ...
    if(buyPrice >= currentPrice) {
        ...
        authedClient.placeOrder(params, (error, response, order) => {
            if (error) {
                ...
                fs.writeFileSync('bin/market.json', JSON.stringify(market));
            }
        });
    }
    console.log("---");
...
async function sell(data, market, trans) {
    let currentPrice = parseFloat(data.price);
    let lastPrice = parseFloat(market.last_price);
    let allocation = parseFloat(market.allocation);
    let spread = parseFloat(market.spread);
    ...
    const balances = await authedClient.getAccounts();
    const assetBalance = parseFloat(balances.filter(el => { return el.currency == 'ADA' })[0].balance);
    ...
    if(sellPrice <= currentPrice) {
        ...
        const params = {
            side: 'sell',
            size: assetBalance,
            product_id: market.crypto, // SUSHI-EUR
            type : 'market'
        };
        authedClient.placeOrder(params, (error, response, order) => {
            if (error) {
